...
array_type: type_list array_variable_decl array_rhs;
array_variable_decl: IDENTIFIER OPEN_SQUARE_BRACKET size CLOSE_SQUARE_BRACKET;
...
parameter: type_list (IDENTIFIER | array_variable_decl);
...
if_stmt: if_part elif_list else_list;
if_part: IF OPEN_ROUND_BRACKET expression CLOSE_ROUND_BRACKET nullable_newline statement;
...
expression7: MINUS expression7 | expression8;
expression8: expression8 index_operators_expr | expression9;
expression9: IDENTIFIER | NUMBER_LITERAL | TRUE | FALSE | STRING_LITERAL | array_literal | index_operators_expr | function_call_stmt | sub_expr;

relational_sign: EQUAL | EQUAL_STRING | NEQ | LESS | GREATER | LEQ | GEQ;

index_operators_expr: (IDENTIFIER | function_call_stmt) OPEN_SQUARE_BRACKET index_operators CLOSE_SQUARE_BRACKET;
